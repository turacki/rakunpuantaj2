...
import { Users, ClipboardList, BarChart3, LogOut, RefreshCw, KeyRound, Eye, EyeOff, X, ArrowRight, Coins, LayoutDashboard, Calculator, ChevronDown, ChevronRight, Package, ShoppingCart } from 'lucide-react';
...
type TabType = 'personnel' | 'puantaj' | 'reports' | 'tips' | 'portal' | 'accounting' | 'inventory' | 'orderPrep';
...
const App: React.FC = () => {
  ...
  const handleClick = () => {
    const availableNames = users.length > 0 ? users.map(u => u.name.split(' ')[0]) : ['Dostum'];
    const randomName = availableNames[Math.floor(Math.random() * availableNames.length)];
    
    const newBubble: Bubble = {
      id: Math.random() + Date.now(),
      content: `Rakun ❤️ ${randomName}`,
      rnd: Math.random()
    };
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 font-sans relative overflow-hidden">
        {bubbles.map(b => (
          <div key={b.id} className="bubble-effect bg-white/95 backdrop-blur-md border border-white/20 px-10 py-5 rounded-full font-black text-indigo-700 text-xl md:text-2xl uppercase tracking-widest shadow-2xl" style={{ '--rnd': b.rnd } as any}>
            {b.content}
          </div>
        ))}
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row relative">
      {bubbles.map(b => (
        <div key={b.id} className="bubble-effect bg-white border border-indigo-100 px-10 py-5 rounded-full font-black text-indigo-700 text-xl md:text-2xl uppercase tracking-widest shadow-xl" style={{ '--rnd': b.rnd } as any}>
          {b.content}
        </div>
      ))}
      ...
      <nav className="w-full lg:w-80 bg-white border-b lg:border-r border-slate-100 flex flex-col lg:sticky lg:top-0 lg:h-screen z-10 shrink-0">
        <div className="p-6 lg:p-10">
          <div className="flex items-center gap-4 mb-2">
            <div className="w-10 h-10 rounded-xl overflow-hidden bg-white flex items-center justify-center shadow-lg shadow-slate-200 border border-slate-50">
               <img src={LOGO_URL} alt="Rakun" className="w-8 h-8 object-contain" />
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-800 tracking-tight leading-none">Dükkan Paneli</h2>
              <p className="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-1">Puantaj Pro</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 lg:p-8 mt-auto border-t border-slate-50 bg-white">
          <div className="flex items-center gap-3 md:gap-4 mb-4 lg:mb-8 bg-slate-50 p-3 lg:p-4 rounded-3xl border border-slate-100">
            <img src={currentUser.avatar} className="w-8 h-8 lg:w-10 lg:h-10 rounded-xl lg:rounded-2xl border-2 border-white shadow-sm" alt="profile" />
