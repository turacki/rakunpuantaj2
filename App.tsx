...
const App: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoadError(null);
    try {
      const [u, e, orders] = await Promise.all([
        db.getUsers(), 
        db.getEntries(),
        db.getInventoryOrders()
      ]);
      ...
      setUsers(finalUsers);
      setEntries(e);
      setPendingOrdersCount(orders.filter(o => o.status !== 'COMPLETED').length);
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col lg:flex-row relative">
          ...
          {(currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.BOSS) && (
            <>
              ...
              <button
                ...
                className={`flex-1 lg:w-full flex items-center justify-between gap-3 px-4 py-3 lg:py-4 rounded-2xl transition-all whitespace-nowrap ${activeTab === 'inventory' ? 'bg-indigo-900 text-white shadow-xl shadow-indigo-100' : 'text-slate-500 hover:bg-slate-50 font-bold'}`}
              >
                <div className="flex items-center gap-3">
                ...
                {pendingOrdersCount > 0 && (
                  <span className="bg-red-500 text-white text-[10px] font-black px-2 py-0.5 rounded-full min-w-[20px] text-center shadow-lg shadow-red-500/30">
                    {pendingOrdersCount}
                  </span>
                )}
              </button>
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-12 lg:overflow-y-auto lg:h-screen custom-scrollbar mobile-scroll-fix z-20 relative">
        <div className="relative z-10">
          ...
          {activeTab === 'inventory' && (currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.BOSS) && (
            <AdminInventory onOrdersUpdate={() => {
              db.getInventoryOrders().then(orders => {
                setPendingOrdersCount(orders.filter(o => o.status !== 'COMPLETED').length);
              });
            }} />
          )}
          {activeTab === 'orderPrep' && <InventoryOrderPrep 
