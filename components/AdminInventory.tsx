...
const AdminInventory: React.FC = () => {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'orders' | 'items' | 'categories'>('orders');
        ...
        <div className="space-y-4">
          {orders.length === 0 ? (
            <div className="bg-white p-12 rounded-[2rem] text-center border-2 border-dashed border-slate-100">
              ...
              <p className="text-slate-400 font-bold uppercase tracking-widest text-sm">Henüz sipariş bildirimi yok</p>
            </div>
          ) : (
            orders.map(order => {
              const shoppingList = getWholesalerShoppingList(order);
              return (
                <div key={order.id} className={`bg-white rounded-[2rem] shadow-sm border overflow-hidden transition-all ${order.status === 'COMPLETED' ? 'border-emerald-100 opacity-75' : 'border-slate-100'}`}>
                  <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-50">
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${order.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-500' : 'bg-amber-50 text-amber-500'}`}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {order.status === 'PENDING' ? (
                        <button 
                          ...
                          className="px-6 py-3 bg-emerald-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-600 transition-all flex items-center gap-2"
                        >
                          <CheckCircle2 size={16} /> Tamamlandı İşaretle
                        </button>
                      ) : (
                        <button 
