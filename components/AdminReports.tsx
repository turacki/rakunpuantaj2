...
const AdminReports: React.FC<Props> = ({ users, entries }) => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `PuantajPro_Yedek_${getLocalDateString()}.json`;
      a.click();
  ...
  const shopSummary = useMemo(() => {
    // ÖNEMLİ: BOSS rolündekileri dükkan dengesinden çıkarıyoruz
    const nonBossBalances = staffBalances.filter(b => b.role !== UserRole.BOSS);
    const totalReceivables = nonBossBalances.filter(b => b.balance > 0).reduce((acc, curr) => acc + curr.balance, 0);
    const totalPayables = nonBossBalances.filter(b => b.balance < 0).reduce((acc, curr) => acc + Math.abs(curr.balance), 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-24 max-w-[1200px] mx-auto px-4 lg:px-0">
      ...
      {selectedUser === 'all' ? (
                  ...
                  {staffBalances.map(staff => (
                    <tr key={staff.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="hidden md:table-cell px-10 py-5 text-center">
                        <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${staff.role === UserRole.ADMIN ? 'bg-amber-100 text-amber-700' : staff.role === UserRole.BOSS ? 'bg-fuchsia-100 text-fuchsia-700' : 'bg-slate-100 text-slate-500'}`}>
                          {staff.role}
                        </span>
                      </td>
        ...
        <div className="space-y-6 animate-in slide-in-from-bottom-4">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm">
            <button onClick={() => setCurrentCalendarDate(new Date(currentCalendarDate.getFullYear(), currentCalendarDate.getMonth() - 1, 1))} className="p-3 hover:bg-slate-50 rounded-2xl transition-colors"><ChevronLeft /></button>
            ...
            <button onClick={() => setCurrentCalendarDate(new Date(currentCalendarDate.getFullYear(), currentCalendarDate.getMonth() + 1, 1))} className="p-3 hover:bg-slate-50 rounded-2xl transition-colors"><ChevronRight /></button>
          </div>
      ...
      {/* GENEL MAĞAZA ÖZETİ - BOSS harici gerçek personel yükü */}
      <div className="bg-slate-900 p-8 md:p-12 rounded-[3rem] text-white shadow-2xl relative overflow-hidden mt-12">
        ...
        <h3 className="text-2xl font-black mb-10 flex items-center gap-4">
           ...
           Genel Mağaza Personel Dengesi (Net Maliyet)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-slate-500 font-bold italic">Mağazanın gerçek personele olan toplam borcu.</p>
          </div>
          <div className="space-y-3">
          ...
          <div className="space-y-3 bg-white/5 p-8 rounded-[2.5rem] border border-white/10 shadow-inner">
            ...
            <p className="text-[11px] text-slate-400 font-bold italic">Ödenmesi gereken net gerçek personel tutarı.</p>
          </div>
        </div>
        <p className="mt-8 text-[10px] font-bold text-slate-500 italic">* BOSS statüsündeki kullanıcılar bu hesaplamalara dahil edilmez.</p>
      </div>
    </div>
  );
};
