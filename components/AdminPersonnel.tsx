      ...
      {isAdding && (
        <div className="bg-white p-10 rounded-[3rem] border-2 border-indigo-100 shadow-sm animate-in zoom-in-95">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-2">Bölüm</label>
              <select className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold appearance-none border border-transparent focus:border-indigo-500 outline-none" value={editForm.department || 'NORMAL'} onChange={e => setEditForm({ ...editForm, department: e.target.value as 'NORMAL' | 'KITCHEN' | 'CHEF' })}>
                <option value="NORMAL">Normal Personel</option>
                <option value="KITCHEN">Mutfak Personeli</option>
                <option value="CHEF">Şef (Özel Yevmiye)</option>
              </select>
            </div>
            <div className="space-y-1">
              ...
              <select className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold appearance-none border border-transparent focus:border-indigo-500 outline-none" value={editForm.role} onChange={e => setEditForm({ ...editForm, role: e.target.value as UserRole })}>
                ...
                <option value={UserRole.BOSS}>Patron (BOSS)</option>
              </select>
            </div>
          </div>
              ...
              if (editForm.name && editForm.password) {
                ...
                try {
                  const newUser: User = { 
                    ...
                    department: editForm.department || 'NORMAL',
                ...
                <td className="px-10 py-6">
                   {editingId === user.id ? (
                    <div className="flex flex-col gap-2">
                      ...
                      <select className="bg-white border-2 border-indigo-500 rounded-xl px-4 py-2 text-xs font-black" value={editForm.department || 'NORMAL'} onChange={e => setEditForm({...editForm, department: e.target.value as 'NORMAL' | 'KITCHEN' | 'CHEF'})}>
                    ...
                    <div className="flex flex-col gap-1">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${getRoleBadge(user.role)}`}>
                        {user.role}
                      </span>
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${user.department === 'KITCHEN' ? 'bg-orange-100 text-orange-600' : user.department === 'CHEF' ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>
                        {user.department === 'KITCHEN' ? 'MUTFAK' : user.department === 'CHEF' ? 'ŞEF' : 'NORMAL'}
                      </span>
                    </div>
                  )}
                </td>
