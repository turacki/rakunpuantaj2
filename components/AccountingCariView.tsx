...
const AccountingCariView: React.FC<Props> = ({ wholesalers, transactions, selectedId, onRefresh, setSelectedId }) => {
  ...
  const [wForm, setWForm] = useState<Partial<Wholesaler> & { openingBalance: string }>({ 
    name: '', 
    phone: '', 
    openingBalance: '',
    companyTitle: '',
  ...
  const handleAddWholesaler = async () => {
    ...
    try {
      ...
      await db.upsertWholesaler({ 
        ...
        contactPerson: wForm.contactPerson?.trim(),
        companyTitle: wForm.companyTitle?.trim(),
        iban: wForm.iban?.trim(),
        salesRep: wForm.salesRep?.trim(),
        defaultMaturityDays: wForm.defaultMaturityDays ? Number(wForm.defaultMaturityDays) : undefined,
        taxOffice: wForm.taxOffice?.trim(),
        taxNumber: wForm.taxNumber?.trim()
      });
  ...
  const handleEditWholesaler = (w: Wholesaler) => {
    setWForm({
      ...
      companyTitle: w.companyTitle || '',
      iban: w.iban || '',
      salesRep: w.salesRep || '',
      defaultMaturityDays: w.defaultMaturityDays,
      taxOffice: w.taxOffice || '',
  ...
  const transactionsWithBalances = useMemo(() => {
    ...
    const sorted = transactions
      .filter(t => t.wholesalerId === selectedId)
      .sort((a, b) => {
        const dateA = new Date(a.date).getTime();
        const dateB = new Date(b.date).getTime();
        if (dateA !== dateB) return dateA - dateB;
        return Number(a.id.split('-')[0]) - Number(b.id.split('-')[0]);
      });
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden min-h-[600px] flex flex-col">
            <div className="p-8 bg-slate-50/50 border-b border-slate-50 flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={() => {
                  const w = wholesalers.find(wh => wh.id === selectedId);
                  const defaultDays = w?.defaultMaturityDays;
                  ...
                  setTForm({ type: 'PURCHASE', date: getLocalDateString(), dueDate });
                  setIsTModalOpen(true);
                }} className="bg-slate-900 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-200"><Plus size={16}/> İşlem Ekle</button>
              </div>
            </div>
            <div className="flex-1 overflow-x-auto">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-50">
                  {[...transactionsWithBalances].reverse().map((t, idx) => {
                    return (
                      <tr key={`${t.id}-${idx}`} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-8 py-5 text-right font-black text-slate-800">
                           {t.runningBalance.toLocaleString()} TL
                        </td>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md z-[9999] flex items-center justify-center p-6">
          <div className="bg-white rounded-[3rem] p-10 w-full max-w-md shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => { setIsWModalOpen(false); setWForm({ name: '', phone: '', openingBalance: '', companyTitle: '', iban: '', salesRep: '', defaultMaturityDays: undefined, taxOffice: '', taxNumber: '', contactPerson: '' }); }}><X/></button>
            </div>
            <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-2 custom-scrollbar">
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Toptancı Adı (Görünen)</label>
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Şirket Ünvanı (Resmi)</label>
                <input type="text" placeholder="Şirket Ünvanı" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.companyTitle} onChange={e => setWForm({...wForm, companyTitle: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Vergi Dairesi</label>
                  <input type="text" placeholder="V. Dairesi" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.taxOffice} onChange={e => setWForm({...wForm, taxOffice: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Vergi No</label>
                  <input type="text" placeholder="V. No" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.taxNumber} onChange={e => setWForm({...wForm, taxNumber: e.target.value})} />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">IBAN No</label>
                <input type="text" placeholder="TR00..." className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.iban} onChange={e => setWForm({...wForm, iban: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Satış Temsilcisi</label>
                  <input type="text" placeholder="İsim Soyisim" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.salesRep} onChange={e => setWForm({...wForm, salesRep: e.target.value})} />
                </div>
                <div className="space-y-1">
                  <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Telefon</label>
                  <input type="text" placeholder="Telefon" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.phone} onChange={e => setWForm({...wForm, phone: e.target.value})} />
                </div>
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-black text-slate-400 uppercase ml-2 tracking-widest">Default Vade (Gün)</label>
                <input type="number" placeholder="Örn: 15" className="w-full px-6 py-4 bg-slate-50 rounded-2xl font-bold outline-none border-2 border-transparent focus:border-emerald-500 transition-all" value={wForm.defaultMaturityDays || ''} onChange={e => setWForm({...wForm, defaultMaturityDays: e.target.value ? Number(e.target.value) : undefined})} />
              </div>
              {!wForm.id && (
                <div className="space-y-1">
