...
import { Calendar, ChevronLeft, ChevronRight, Info, Wallet, TrendingUp, ReceiptText, MessageSquare, KeyRound, Save, CheckCircle2, AlertCircle, RefreshCw, Coins, ShoppingCart, Banknote, EyeOff } from 'lucide-react';
...
const StaffPortal: React.FC<Props> = ({ user, entries, setUser }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      earnings: filtered
        .filter(e => e.amount > 0 && e.type !== 'TIP')
        .reduce((acc, curr) => acc + curr.amount, 0),
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-32 px-4 md:px-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <div className="bg-white p-2 md:p-4 rounded-3xl border border-slate-100 flex items-center gap-4 shadow-sm w-full md:w-auto justify-between">
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-2 hover:bg-slate-50 rounded-xl transition-colors"><ChevronLeft /></button>
          ...
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="p-2 hover:bg-slate-50 rounded-xl transition-colors"><ChevronRight /></button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        {/* Aylık Kazanç Kartı */}
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between transition-all hover:border-emerald-200">
          <div>
          ...
          <p className="text-2xl font-black text-emerald-600 mt-2">+{stats.earnings.toLocaleString()} TL</p>
        </div>

        {/* Harcamalar Kartı (YENİ) */}
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between transition-all hover:border-orange-200">
          <div>
            <div className="bg-orange-50 w-8 h-8 rounded-xl flex items-center justify-center mb-3 text-orange-600"><ShoppingCart className="w-4 h-4" /></div>
            <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest">Aylık Harcamalar (Yemek vb.)</p>
          </div>
          <p className="text-2xl font-black text-orange-600 mt-2">-{stats.expenses.toLocaleString()} TL</p>
        </div>

        {/* Alınan Ödemeler Kartı */}
        <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between transition-all hover:border-fuchsia-200">
          <div>
            <div className="bg-fuchsia-50 w-8 h-8 rounded-xl flex items-center justify-center mb-3 text-fuchsia-600"><Banknote className="w-4 h-4" /></div>
            <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest">Alınan Ödemeler / Avans</p>
          </div>
          <p className="text-2xl font-black text-fuchsia-600 mt-2">-{stats.payments.toLocaleString()} TL</p>
        </div>
      </div>
              ...
              >
                ...
                <div className="flex flex-col gap-1 mt-1 flex-1 md:w-full overflow-visible relative items-end md:items-stretch">
                  {dayEntries.map(e => (
                    ...
                    >
                      <div 
                        className={`text-[10px] md:text-[9px] font-black px-3 py-2 md:py-1.5 rounded-xl border flex items-center justify-between gap-2 transition-all cursor-help ${e.excludeFromTotals ? 'opacity-50 grayscale' : ''} ${
                          e.type === 'TIP'
                      ...
                      >
                        ...
                        {e.excludeFromTotals && <EyeOff size={10} className="text-slate-400" />}
