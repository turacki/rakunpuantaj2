...
const AdminPuantaj: React.FC<Props> = ({ users, entries, setEntries }) => {
  ...
  const [modal, setModal] = useState<ModalState>({ 
    isOpen: false, userId: '', type: 'CUSTOM', amount: '', hours: '', note: '', title: '', excludeFromTotals: false, selectedItems: {}
  });
  ...
  // Pencere açıldığında en üste kaydır
  useEffect(() => {
    if (modal.isOpen && modalScrollRef.current) {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        if (settings.staffMenuItems) {
          setStaffMenuItems(JSON.parse(settings.staffMenuItems));
        } else {
          // Varsayılan menü
          const defaultMenu: StaffMenuItem[] = [
            { id: '1', name: 'Bira 50cl', price: 100 },
            { id: '2', name: 'Bira 30cl', price: 60 },
            { id: '3', name: 'İthal Bira', price: 130 },
            { id: '4', name: '6cl Kokteyl', price: 130 },
            { id: '5', name: 'Yüksek Alkol Kokteyl', price: 160 },
            { id: '6', name: 'Shot', price: 90 },
            { id: '7', name: 'Viski Shot', price: 100 },
            { id: '8', name: 'Şarap', price: 150 },
            { id: '9', name: 'Viski Tek', price: 100 },
            { id: '10', name: 'Duble', price: 160 },
            { id: '11', name: 'Yemek Tabak', price: 180 },
            { id: '12', name: 'Atıştırmalık', price: 150 },
          ];
          setStaffMenuItems(defaultMenu);
          await db.upsertSetting('staffMenuItems', JSON.stringify(defaultMenu));
        }
      } catch (e) {
  ...
  const updateItemQuantity = (itemId: string, delta: number) => {
    setModal(prev => {
      ...
      const newSelectedItems = { ...prev.selectedItems, [itemId]: newQty };
      
      // Toplam tutarı hesapla
      let totalAmount = 0;
      Object.entries(newSelectedItems).forEach(([id, qty]) => {
        const item = staffMenuItems.find(i => i.id === id);
        if (item) totalAmount += item.price * qty;
      });

      // Notu güncelle
      const selectedItemsList = Object.entries(newSelectedItems)
        .filter(([_, qty]) => qty > 0)
        .map(([id, qty]) => {
          const item = staffMenuItems.find(i => i.id === id);
      ...
      return {
        ...
        note: selectedItemsList || prev.note
      };
    });
  };
  ...
  const syncToAdisyo = async (department: 'NORMAL' | 'KITCHEN') => {
    ...
    try {
      ...
      // Seçilen tarih ve şu anki yerel saat ile bir tarih objesi oluşturuyoruz
      const localExpenseDate = new Date(year, month - 1, day, now.getHours(), now.getMinutes(), now.getSeconds());
      // Adisyo API'si UTC zamanı beklediği için UTC ISO formatına çevirip 'Z'yi kaldırıyoruz
      const expenseDate = localExpenseDate.toISOString().replace('Z', '');
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-[1400px] mx-auto relative">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <button 
                  ...
                  className="p-2 bg-slate-50 rounded-xl text-slate-400 hover:bg-indigo-50 hover:text-indigo-600 transition-all shadow-sm border border-slate-100"
                  title="Genel Ayarlar"
                >
                  <Settings2 size={18} />
                </button>
      ...
      {modal.isOpen && (
        <div ref={modalScrollRef} className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-start justify-center p-6 pt-12 md:pt-20 overflow-y-auto no-scrollbar animate-in fade-in">
          <div className={`bg-white rounded-[2.5rem] shadow-2xl w-full ${modal.type === 'EXPENSE' ? 'max-w-2xl' : 'max-w-md'} overflow-hidden animate-in zoom-in-95 mb-12 flex flex-col`}>
            <div className="p-8 overflow-y-auto no-scrollbar">
              ...
              {modal.type === 'EXPENSE' && (
                <div className="mb-8 space-y-6">
                  {/* Seçilen Ürünler Çipleri */}
                  {Object.entries(modal.selectedItems).some(([_, qty]) => qty > 0) && (
                    <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100 animate-in slide-in-from-top-2">
                      <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest mb-3 ml-1">Seçilen Ürünler</h4>
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(modal.selectedItems).map(([itemId, qty]) => {
                          const item = staffMenuItems.find(i => i.id === itemId);
                          if (!item || qty <= 0) return null;
                          return (
                            <div key={itemId} className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-indigo-100 shadow-sm animate-in zoom-in-95">
                              <span className="text-xs font-bold text-slate-700">{qty}x {item.name}</span>
                              <button 
                  ...
                  {/* Ürün Havuzu (Çipler) */}
                  <div>
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 ml-2">Ürün Seçin (Dokunarak Ekle)</h4>
                    <div className="flex flex-wrap gap-2">
                      {staffMenuItems.map(item => (
                        <button 
                          key={item.id}
                          onClick={() => updateItemQuantity(item.id, 1)}
                          className="px-4 py-3 bg-slate-50 rounded-2xl border border-slate-100 text-xs font-bold text-slate-700 hover:border-indigo-500 hover:text-indigo-600 hover:bg-white transition-all active:scale-95 flex flex-col items-start gap-1 min-w-[100px] shadow-sm"
                        >
                          <span>{item.name}</span>
                          <span className="text-[9px] font-black text-indigo-400 uppercase">{item.price} TL</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div className={`grid ${showHoursInModal ? 'grid-cols-2' : 'grid-cols-1'} gap-4`}>
                  <div className="space-y-1">
                    ...
                    <input type="number" disabled={busy} className="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white rounded-2xl outline-none font-black text-2xl transition-all" value={modal.amount} onChange={e => setModal({...modal, amount: e.target.value})} />
                  </div>
                  {showHoursInModal && (
                    <div className="space-y-1">
                ...
                {(modal.type === 'PAYMENT' || modal.type === 'CUSTOM') && (
                  <div 
                    className="flex items-center gap-3 p-4 bg-slate-50 rounded-2xl border border-slate-100 cursor-pointer hover:bg-slate-100 transition-all" 
                    onClick={() => {
                      const newExclude = !modal.excludeFromTotals;
                      setModal({
                        ...modal, 
                        excludeFromTotals: newExclude,
                        note: newExclude ? 'RT' : modal.note
                      });
                    }}
                  >
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl overflow-hidden animate-in zoom-in-95 max-h-[90vh] flex flex-col">
            <div className="p-8 overflow-y-auto no-scrollbar">
              <div className="flex justify-between items-center mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-black text-slate-800 tracking-tight">Genel Ayarlar</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Yevmiye ve Menü Fiyatlarını Yönet</p>
                  </div>
                </div>
                <button onClick={() => setIsSettingsOpen(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400"><X size={28} /></button>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                ...
                <div className="space-y-6">
                  <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                    <HandCoins className="w-4 h-4 text-emerald-500" /> Yevmiye Tutarları
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Normal 5K</label>
                      <input type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-black text-sm focus:border-indigo-500 transition-all" value={config5K} onChange={e => updateSetting('config5K', e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Normal 8K</label>
                      <input type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-black text-sm focus:border-indigo-500 transition-all" value={config8K} onChange={e => updateSetting('config8K', e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Şef 5K</label>
                      <input type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-black text-sm focus:border-indigo-500 transition-all" value={configChef5K} onChange={e => updateSetting('configChef5K', e.target.value)} />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Şef 8K</label>
                      <input type="number" className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-xl outline-none font-black text-sm focus:border-indigo-500 transition-all" value={configChef8K} onChange={e => updateSetting('configChef8K', e.target.value)} />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-black text-slate-800 uppercase tracking-widest flex items-center gap-2">
                      <ShoppingBag className="w-4 h-4 text-orange-500" /> Personel Menüsü
                    </h4>
                    <button 
                      onClick={() => {
                        const newItem: StaffMenuItem = { id: Date.now().toString(), name: 'Yeni Ürün', price: 0 };
                        const newMenu = [...staffMenuItems, newItem];
                        setStaffMenuItems(newMenu);
                        db.upsertSetting('staffMenuItems', JSON.stringify(newMenu));
                      }}
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 no-scrollbar">
                    {staffMenuItems.map((item, idx) => (
                      <div key={item.id} className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl border border-slate-100 group">
                        <input 
                          type="text" 
                          className="flex-1 bg-transparent border-none outline-none text-xs font-bold text-slate-700 px-2" 
                          value={item.name} 
                          onChange={e => {
                            const newMenu = [...staffMenuItems];
                        ...
                        <div className="relative w-24">
                          <input 
                            type="number" 
                            className="w-full bg-white border border-slate-200 rounded-lg py-1.5 pl-2 pr-6 text-xs font-black text-indigo-600 outline-none focus:border-indigo-500" 
                            value={item.price} 
                            onChange={e => {
                              const newMenu = [...staffMenuItems];
                              newMenu[idx].price = parseFloat(e.target.value) || 0;
                              setStaffMenuItems(newMenu);
                              db.upsertSetting('staffMenuItems', JSON.stringify(newMenu));
                            }}
                          />
                          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[8px] font-black text-slate-300">TL</span>
                        </div>
                        <button 
                          onClick={() => {
                            const newMenu = staffMenuItems.filter((_, i) => i !== idx);
                            setStaffMenuItems(newMenu);
                            db.upsertSetting('staffMenuItems', JSON.stringify(newMenu));
                          }}
                          className="p-2 text-slate-300 hover:text-red-500 transition-colors"
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button onClick={() => setIsSettingsOpen(false)} className="bg-indigo-600 text-white px-12 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">Kapat</button>
            </div>
          </div>
        </div>
      )}
