...
import { Coffee, Banknote, ShoppingBag, Trash2, Calendar, Edit2, X, Check, HandCoins, RefreshCw, ClipboardList, AlertTriangle, Check as CheckIcon, AlertCircle, Settings2, Send, CheckCircle2 } from 'lucide-react';
  ...
  const calculateDailyTotal = () => {
    return todaysEntries
      .filter(e => {
        ...
        return e.amount > 0 && user?.role !== UserRole.BOSS;
      })
  ...
  const syncToAdisyo = async (department: 'NORMAL' | 'KITCHEN') => {
    const total = todaysEntries
      .filter(e => {
        ...
        return e.amount > 0 && user?.role !== UserRole.BOSS && (department === 'KITCHEN' ? isUserKitchen : !isUserKitchen);
      })
      .reduce((acc, curr) => acc + curr.amount, 0);
    ...
    try {
      ...
      let shiftId = 0;
      try {
        const activeShift = await adisyoService.getActiveShift();
        console.log("Active Shift:", activeShift);
        if (activeShift && activeShift.id) {
          shiftId = activeShift.id;
        }
      } catch (e) {
        console.warn("Could not get active shift, using 0", e);
      }
