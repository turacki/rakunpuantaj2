...
import { Database, RefreshCw, Download, AlertCircle, CheckCircle2, Terminal, Calendar, BarChart2, TrendingUp, Settings, X, ChevronDown, Bot, LayoutDashboard } from 'lucide-react';
...
const AdisyoModule: React.FC = () => {
  ...
  const handleFetchData = async () => {
    ...
    try {
      ...
      if (includeTrend) {
        setMessage('Günlük trend verileri hazırlanıyor (Bu işlem biraz sürebilir)...');
      ...
      if (data && Array.isArray(data) && data.length > 0 && !selectedCategory) {
  ...
  // Seçili kategori için günlük trend verisi
  const selectedCategoryTrend = useMemo(() => {
    if (!dailyData || dailyData.length === 0 || !selectedCategory) return [];

    return dailyData.map(day => {
      const categoryData = day.categories.find((c: any) => c.name === selectedCategory);
      return {
        date: day.date,
        sum: categoryData ? categoryData.sum : 0,
        quantity: categoryData ? categoryData.quantity : 0
      };
    });
  }, [dailyData, selectedCategory]);
      ...
      {activeTab === 'analyst' ? (
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
            ...
            {includeTrend && chartData.length > 0 && (
              <div className="mt-6 space-y-2">
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Kategoriler</p>
        ...
        <div className="lg:col-span-3 space-y-6">
          {chartData.length > 0 ? (
            <>
              ...
              {/* Daily Trend Chart */}
              {selectedCategory && selectedCategoryTrend.length > 0 && (
                <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100">
                  <h3 className="font-black text-slate-800 mb-8 flex items-center gap-2 uppercase tracking-widest text-sm">
                    <TrendingUp className="w-4 h-4 text-emerald-600" /> {selectedCategory} - Günlük Satış Trendi
                  </h3>
                  <div className="h-[400px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={selectedCategoryTrend} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                        ...
                        <XAxis 
                          ...
                          tickFormatter={(value) => {
                            ...
                            return `${date.toLocaleDateString('tr-TR', { day: '2-digit', month: '2-digit' })} ${dayName}`;
                          }}
                        />
                        ...
                        <Tooltip 
                          ...
                          labelFormatter={(value) => {
                            ...
                            return `${date.toLocaleDateString('tr-TR', { day: '2-digit', month: 'long', year: 'numeric' })} ${dayName}`;
                          }}
                        />
                        <Line 
                          type="monotone" 
