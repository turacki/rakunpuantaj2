          ...
          <div className="flex-1 p-8 space-y-4 overflow-y-auto custom-scrollbar">
            {upcomingVades.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {upcomingVades.map((p, idx) => {
                  const today = new Date(); today.setHours(0,0,0,0);
                  const isOverdue = new Date(p.dueDate).getTime() < today.getTime();
                  return (
                    <div key={`${p.id}-${idx}`} className={`p-6 rounded-[2rem] border flex items-center justify-between transition-all hover:scale-[1.02] ${isOverdue ? 'bg-red-50 border-red-100' : 'bg-orange-50 border-orange-100'}`}>
                      <div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8">
        ...
        {selectedDay && (
          <div className="mt-8 p-6 bg-slate-50 rounded-[2rem] border border-slate-100 animate-in slide-in-from-bottom-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {unpaidVades.filter(v => v.dueDate === selectedDay).map((v, idx) => (
                <div key={`${v.id}-${idx}`} className="bg-white p-4 rounded-2xl border border-slate-100 flex justify-between items-center">
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-lg p-10">
            ...
            <div className="space-y-4">
              ...
              <select 
                ...
                value={tForm.wholesalerId || ''} 
                onChange={e => {
                  const wId = e.target.value;
                  const w = wholesalers.find(wh => wh.id === wId);
                  let dueDate = tForm.dueDate;
                  
                  if (w?.defaultMaturityDays && w.defaultMaturityDays > 0) {
                    const d = new Date();
                    d.setDate(d.getDate() + w.defaultMaturityDays);
                    dueDate = getLocalDateString(d);
                  }
                  
                  setTForm({...tForm, wholesalerId: wId, dueDate});
                }}
              >
