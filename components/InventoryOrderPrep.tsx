...
const InventoryOrderPrep: React.FC<Props> = ({ currentUser }) => {
  ...
  const handleSubmit = async () => {
    const details = Object.entries(counts)
      ...
      .map(([itemId, count]) => ({ itemId, countedQuantity: count as number }));

    if (details.length === 0) {
      alert("Lütfen en az bir ürün sayımı girin.");
      return;
    }
    ...
    <div className="max-w-2xl mx-auto p-4 pb-32">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-md px-6 z-50">
        <button 
          ...
          disabled={submitting || Object.values(counts).every(v => (v as number) === 0)}
        ...
        >
          {submitting ? (
            ...
            <>
              <Send size={20} />
              SİPARİŞİ TAMAMLA ({Object.values(counts).filter(v => (v as number) > 0).length} Ürün)
            </>
          )}
        </button>
      </div>
