...
export const adisyoService = {
  ...
  ORD_URL: 'https://ord.adisyo.com/api',
  ...
  // Helper to format date to UTC ISO string (Adisyo expects UTC)
  formatToAdisyoDate(date: Date) {
  ...
  async getTopSalesByCategory(startDate: string, endDate: string) {
    return this.requestWithRetry(async () => {
      ...
      // Format dates for Adisyo (expects UTC)
      // We convert local day boundaries to UTC
      const [sYear, sMonth, sDay] = startDate.split('T')[0].split('-').map(Number);
      const startLocal = new Date(sYear, sMonth - 1, sDay, 0, 0, 0);
      const start = this.formatToAdisyoDate(startLocal);

      const [eYear, eMonth, eDay] = endDate.split('T')[0].split('-').map(Number);
      const endLocal = new Date(eYear, eMonth - 1, eDay, 23, 59, 59);
      const end = this.formatToAdisyoDate(endLocal);
  ...
  async saveExpense(expenseData: any) {
    ...
    const formatDate = (date: Date) => {
      return this.formatToAdisyoDate(date);
    };
  ...
  async getOrderForDetail(orderId: string) {
    return this.requestWithRetry(async () => {
      await this.ensureToken();
      let restaurantId = "21276";
      try {
      ...
      const response = await axios.post(`${this.ORD_URL}/orders/GetOrderForDetail`, {
        id: parseInt(orderId)
      }, {
        headers: this.getHeaders(),
      });
