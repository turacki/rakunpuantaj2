...
export const adisyoService = {
  ...
  async getDailySalesByCategory(startDate: string, endDate: string) {
    ...
    // Güvenlik: Çok geniş tarih aralığı API'yi yorabilir, 62 gün sınırı koyalım
    const diffTime = Math.abs(end.getTime() - start.getTime());
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    
    if (diffDays > 62) {
      throw new Error('Trend analizi için lütfen en fazla 62 günlük (2 ay) bir aralık seçin.');
    }
