...
import { User, PuantajEntry, Wholesaler, AccTransaction, InventoryItem, InventoryOrder, InventoryOrderDetail, InventoryCategory, ShiftPlan } from '../types';
...
/**
 * Helper to fetch all rows from a table by paging through them (bypassing the 1000-row limit)
 */
const fetchAll = async (table: string, select: string = '*', orderCol: string = 'id', ascending: boolean = false) => {
  let allData: any[] = [];
  let from = 0;
  let to = 999;
  let hasMore = true;
...
export const db = {
  ...
  getUsers: async (): Promise<User[]> => {
    return withRetry(async () => {
      const data = await fetchAll('users', '*', 'name', true);
      return (data || []).map(u => ({
        id: u.id,
        name: u.name,
        role: u.role,
  ...
  getEntries: async (): Promise<PuantajEntry[]> => {
    return withRetry(async () => {
      const data = await fetchAll('entries', '*', 'date', false);
      return (data || []).map(e => ({
        id: e.id,
        userId: e.user_id,
        type: e.type,
        ...
        date: e.date,
        note: e.note,
        excludeFromTotals: e.exclude_from_totals
      }));
    });
  },
  ...
  upsertEntry: async (entry: PuantajEntry) => {
    return withRetry(async () => {
      const { error } = await supabase.from('entries').upsert({
        ...
        date: entry.date,
        note: entry.note,
        exclude_from_totals: entry.excludeFromTotals
      });
      if (error) throw error;
    });
  },
  ...
  getWholesalers: async (): Promise<Wholesaler[]> => {
    ...
    return (data || []).map(w => ({
      ...
      companyTitle: w.company_title,
      iban: w.iban,
      salesRep: w.sales_rep,
      defaultMaturityDays: w.default_maturity_days,
      taxOffice: w.tax_office,
  ...
  upsertWholesaler: async (w: Wholesaler) => {
    const { error } = await supabase.from('wholesalers').upsert({
      ...
      company_title: w.companyTitle,
      iban: w.iban,
      sales_rep: w.salesRep,
      default_maturity_days: w.defaultMaturityDays,
      tax_office: w.taxOffice,
  ...
  getAccTransactions: async (): Promise<AccTransaction[]> => {
    const data = await fetchAll('acc_transactions', '*', 'date', false);
    return (data || []).map(t => ({
      id: t.id,
  ...
  getInventoryOrders: async (): Promise<InventoryOrder[]> => {
    // Önce siparişleri ve detayları çekelim (Sayfalama ile)
    let allOrders: any[] = [];
    let from = 0;
    let to = 999;
    let hasMore = true;
  ...
  // --- Shift Plan Metodları ---
  getShiftPlans: async (): Promise<ShiftPlan[]> => {
    const { data, error } = await supabase.from('shift_plans').select('*').order('week_start', { ascending: false });
    if (error) return [];
    return (data || []).map(s => ({
      id: s.id,
      weekStart: s.week_start,
      planData: s.plan_data,
      createdAt: s.created_at
    }));
  },

  upsertShiftPlan: async (plan: Partial<ShiftPlan>) => {
