...
export const db = {
  ...
  getEntries: async (): Promise<PuantajEntry[]> => {
    return withRetry(async () => {
      const { data, error } = await supabase.from('entries').select('*').order('date', { ascending: false }).limit(100000);
      if (error) throw error;
      return (data || []).map(e => ({
        id: e.id,
        userId: e.user_id,
  ...
  getAccTransactions: async (): Promise<AccTransaction[]> => {
    const { data, error } = await supabase.from('acc_transactions').select('*').order('date', { ascending: false }).limit(100000);
    if (error) return [];
    return (data || []).map(t => ({
      id: t.id,
